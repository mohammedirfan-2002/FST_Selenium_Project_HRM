...
//		driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(5));
	...
	public void addWork() {
		...
		driver.findElement(By.id("experience_jobtitle")).sendKeys("ASE");
		//locate Date from field and enter value
		driver.findElement(By.id("experience_from_date")).clear();
		driver.findElement(By.id("experience_from_date")).sendKeys("2025-10-02");
		...
		driver.findElement(By.id("experience_comments")).sendKeys("nothing ");
		...
		for(WebElement row :worklist) {
			if(row.getText().contains("IBM")) {
				is_present = true;
			}
		}
		
		Assert.assertTrue(is_present);
	}
	@AfterClass
